...
do
    ...
    EXTRA_CFLAGS="-arch ${ARCH}"
        if [ "${ARCH}" == "x86_64" ] || [ "${ARCH}" == "x86_64h" ]; then
    ...
    LDFLAGS="$LDFLAGS ${OPT_LDFLAGS} -fPIE -mmacosx-version-min=${MINIOSVERSION} -L${OUTPUTDIR}/lib" \
    CFLAGS="$CFLAGS ${EXTRA_CFLAGS} ${OPT_CFLAGS} -fPIE -mmacosx-version-min=${MINIOSVERSION} -I${OUTPUTDIR}/include -isysroot ${DEVELOPER}/Platforms/${PLATFORM}.platform/Developer/SDKs/${PLATFORM}${SDKVERSION}.sdk" \
